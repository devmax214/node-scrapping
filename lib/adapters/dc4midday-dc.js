...
export const dc4middaydcRecent = [
  // Scraper for the recently passed draw (Primary)
  {
    lotteryName:'dc4middaydc',
    regions: ['DC'],
      ...
      jackpot: {
        ...
        transform: async (html) => {
          let jackpot = await getData({
            ...
            path: 'tr:contains("DC 4 Midday") span.jackpot-amount',
            transform: async (html) => {
  ...
  // Scraper for the recently passed draw (Backup)
  {
    lotteryName:'dc4middaydc',
    regions: ['DC'],
    url:"http://www.lotteryusa.com/district-of-columbia/dc-4-midday/",
    data: {
      numbers: {
        path: 'ul.draw-result',
        transform: async (html) => {
...
export const dc4middaydcNext = [
      ...
      date: {
        ...
        transform: async (html) => {
          let nextDate = await getData({
            url: 'https://www.lotterypost.com/game/30',
  ...
  // Scraper for the recently passed draw (Backup)
  {
    ...
    data: {
      jackpot: {
        path: 'tr:contains("DC 4 Midday") span.next-jackpot-amount',
        transform: async (html) => {
      ...
      date: {
        path: 'tr:contains("DC 4 Midday") span.next-draw-date',
        transform: async (html) => {
          const text = html.first().text();
          const date = await moment(text, "ddd, MMM DD, YYYY").format();
          return date;
