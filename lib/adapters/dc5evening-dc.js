...
export const dc5eveningdcRecent = [
  // Scraper for the recently passed draw (Primary)
  {
    lotteryName:'dc5eveningdc',
    regions: ['DC'],
      ...
      jackpot: {
        ...
        transform: async (html) => {
          let jackpot = await getData({
            ...
            path: 'tr:contains("DC 5") span.jackpot-amount',
            transform: async (html) => {
  ...
  // Scraper for the recently passed draw (Backup)
  {
    lotteryName:'dc5eveningdc',
    regions: ['DC'],
    url:"http://www.lotteryusa.com/district-of-columbia/dc-5/",
    data: {
      numbers: {
        path: 'ul.draw-result',
        transform: async (html) => {
...
export const dc5eveningdcNext = [
      ...
      date: {
        ...
        transform: async (html) => {
          let nextDate = await getData({
            url: 'https://www.lotterypost.com/game/359',
  ...
  // Scraper for the recently passed draw (Backup)
  {
    ...
    data: {
      jackpot: {
        path: 'tr:contains("DC 5") span.next-jackpot-amount',
        transform: async (html) => {
      ...
      date: {
        path: 'tr:contains("DC 5") span.next-draw-date',
        transform: async (html) => {
          const text = html.first().text();
          const date = await moment(text, "ddd, MMM DD, YYYY").format();
          return date;
