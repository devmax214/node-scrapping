...
export const pick3eveningtxRecent = [
  // Scraper for the recently passed draw (Primary)
  {
    lotteryName:'pick3eveningtx',
    regions: ['TX'],
    ...
    data: {
      ...
      date: {
        ...
        transform: async (html) => {
          const text = html.eq(1).text().replace("Pick 3 Evening Winning Numbers for ",'');
  ...
  // Scraper for the recently passed draw (Backup)
  {
    lotteryName:'pick3eveningtx',
    regions: ['TX'],
    url:"http://www.lotteryusa.com/texas/evening-pick-3/",
    data: {
      numbers: {
        path: 'ul.draw-result',
        transform: async (html) => {
...
export const pick3eveningtxNext = [
  // Scraper for the recently passed draw (Primary)
  {
    lotteryName:'pick3eveningtx',
    regions: ['TX'],
      ...
      date: {
        ...
        transform: async (html) => {
          let date = await getData({
            url: 'https://www.lotterypost.com/game/436',
  ...
  // Scraper for the recently passed draw (Backup)
  {
    ...
    data: {
      jackpot: {
        path: 'tr:contains("Evening Pick 3") span.next-jackpot-amount',
        transform: async (html) => {
          return parseInt(html.first().clone().children().remove().end().text().trim().replace(/\D/g,""));
        }
      },
      date: {
        path: 'tr:contains("Evening Pick 3") span.next-draw-date',
        transform: async (html) => {
