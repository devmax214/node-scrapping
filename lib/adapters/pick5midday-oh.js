...
export const pick5middayohRecent = [
  // Scraper for the recently passed draw (Primary)
  {
    lotteryName:'pick5middayoh',
    regions: ['OH'],
    url:"https://www.ohiolottery.com/mobile/pick-5",
    data: {
      numbers: {
        path: 'li.pick5:contains("Midday") ul.balls',
      ...
      jackpot: {
        ...
        transform: async (html) => {
          let jackpot = await getData({
            ...
            path: 'tr:contains("idday Pick 5") span.next-draw-date',
            transform: async (html) => {
      ...
      date: {
        path: 'li.pick5:contains("Midday") span.date',
        transform: async (html) => {
  ...
  // Scraper for the recently passed draw (Backup)
  {
    lotteryName:'pick5middayoh',
    regions: ['OH'],
    url:"http://www.lotteryusa.com/ohio/midday-pick-5/",
    data: {
      numbers: {
        path: 'ul.draw-result',
        transform: async (html) => {
...
export const pick5middayohNext = [
  // Scraper for the recently passed draw (Primary)
  {
    lotteryName:'pick5middayoh',
    regions: ['OH'],
    url:"https://www.lotterypost.com/game/409",
    data: {
      jackpot: {
       path: 'div',
        transform: async (html) => {
          let jackpot = await getData({
            url: 'http://www.lotteryusa.com/ohio/',
            path: 'tr:contains("idday Pick 5") span.next-jackpot-amount',
            transform: async (html) => {
              let jackpot = parseInt(html.first().text().trim().replace(/\s.+/g,"").replace(/\D/g,""));
              return jackpot;
            }
