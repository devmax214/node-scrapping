...
export const pick47pmorRecent = [
  // Scraper for the recently passed draw (Primary)
  {
    lotteryName:'pick47pmor',
    regions: ['OR'],
    url:"http://www.oregonlottery.org/games/draw-games/pick-4/past-results",
    data: {
      numbers: {
        path: 'tr:contains("07:00PM")',
      ...
      jackpot: {
        ...
        transform: async (html) => {
          let jackpot = await getData({
            ...
            path: 'tr:contains("Pick 4 7pm") span.jackpot-amount',
            transform: async (html) => {
  ...
  // Scraper for the recently passed draw (Backup)
  {
    lotteryName:'pick47pmor',
    regions: ['OR'],
    url:"http://www.lotteryusa.com/oregon/pick-4-7pm/",
    data: {
      numbers: {
        path: 'ul.draw-result',
        transform: async (html) => {
...
export const pick47pmorNext = [
    ...
    data: {
      jackpot: {
        ...
        transform: async (html) => {
          let jackpot = await getData({
            url: 'http://www.lotteryusa.com/oregon/',
            path: 'tr:contains("Pick 4 7pm") span.next-jackpot-amount',
            transform: async (html) => {
      ...
      date: {
        ...
        transform: async (html) => {
          let nextDate = await getData({
            url: 'https://www.lotterypost.com/game/163',
  ...
  // Scraper for the recently passed draw (Backup)
  {
    ...
    data: {
      ...
      date: {
        path: 'tr:contains("Pick 4 7pm") span.next-draw-date',
        transform: async (html) => {
          const text = html.first().text();
          const date = await moment(text, "ddd, MMM DD, YYYY").format();
          return date;
