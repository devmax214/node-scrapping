...
export const pick3nighttxRecent = [
  // Scraper for the recently passed draw (Primary)
  {
    lotteryName:'pick3nighttx',
    regions: ['TX'],
    ...
    data: {
      ...
      date: {
        ...
        transform: async (html) => {
          const text = html.eq(1).text().replace("Pick 3 Night Winning Numbers for ",'');
  ...
  // Scraper for the recently passed draw (Backup)
  {
    lotteryName:'pick3nighttx',
    regions: ['TX'],
    url:"http://www.lotteryusa.com/texas/pick-3/",
    data: {
      numbers: {
        path: 'ul.draw-result',
        transform: async (html) => {
      ...
      date: {
        ...
        transform: async (html) => {
          let date = await getData({
            url: 'https://www.lotterypost.com/game/185',
