...
export const pick3daytxRecent = [
  // Scraper for the recently passed draw (Primary)
  {
    lotteryName:'pick3daytx',
    regions: ['TX'],
    ...
    data: {
      ...
      date: {
        ...
        transform: async (html) => {
          const text = html.first().text().replace("Pick 3 Day Winning Numbers for ",'');
  ...
  // Scraper for the recently passed draw (Backup)
  {
    lotteryName:'pick3daytx',
    regions: ['TX'],
    url:"http://www.lotteryusa.com/texas/midday-pick-3/",
    data: {
      numbers: {
        path: 'ul.draw-result',
        transform: async (html) => {
      ...
      date: {
        ...
        transform: async (html) => {
          let date = await getData({
            url: 'https://www.lotterypost.com/game/184',
